...
// GET BOOKS BY ORDER IN REQ.QUERY
router.get("/getBooks", async (req, res) => {
  try {
    ...
    switch (orderBy) {
      case 'author':
        ITENS = await BookModel.find().select("-_id -__v").sort({
          author: 1
        });
        break;
      ...
      default:
        await BookModel.find().select("-_id -__v").sort({
          author: -1
        });
        break;
    }
...
router.post("/postBook", async (req, res) => {
  try {
    // Validation
    const ITENS = await BookModel.find({
      name: req.body.name,
    ...
    if (ITENS.length == 0) {
      // Handling integer ID:
      const auxList = await BookModel.find().sort({
        "_id": -1
      }).limit(1)
      let auxObj = req.body
      if (auxList.length === 0) {
      ...
      const saveJson = new BookModel(auxObj)
      await saveJson.save()
...
router.patch("/updateOne", async (req, res) => {
  try {
    ...
    const updatedItem = await BookModel.updateOne({
      id: updateId
    }, updateObj);
  ...
  try {
    ...
    if (INVENTORY) {
      if (INVENTORY.quantity > 0) {
        ...
        const delInv = await BookInventoryModel.deleteOne({
          id: INVENTORY.id
        });
        auxInvDel = true
      }
    ...
    // delete book
    const del = await BookModel.deleteOne({
      id: parsedId
    });

    // change message depending on inventory delete
    let msg = "Deleted book."
    if (auxInvDel) msg = "Deleted book and inventory registries."
    res.send({
      status: true,
      message: msg
    })
